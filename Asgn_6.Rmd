...
```{r test boston, eval=TRUE, echo=FALSE}
...
```{r test boston black, eval=TRUE, echo=FALSE}
...
```{r plot the map, eval=TRUE, echo=FALSE}
...
``` {r lets get interactive, eval=FALSE, echo=FALSE}
...
```{r pop it up, eval=TRUE, echo=FALSE}
...
```{r pop with variables from leaflet, eval=TRUE, echo=FALSE}
...
``` {r save the map, eval=FALSE, echo=FALSE}
saveWidget(Black_map3, file="inter_black.html")
```

# Let's Do ACS

``` {r ACS map, eval=TRUE, echo=FALSE}
...
``` {r read the transit data, eval=TRUE, echo=FALSE}
...
#Convert point data (lag,log) into sf points (geometry!)

station_sf <- st_as_sf(transit_stations, 
                          coords = c("stop_lon", "stop_lat"), 
                          crs = "WGS84")

#Convert back to Csv file and read it in as a data frame
...
#Displaying markers
``` {r the markers, eval=TRUE, echo=FALSE}
...
``` {r set the extents, eval=TRUE, echo=FALSE}
...
```{r points to circle, eval=TRUE, echo=FALSE}
